...
Vue.mixin({
    ...
    computed: {
        ...mapGetters(['isLoggedOn', 'boughtCourses', 'coursesOnBasket', 'favouriteCourses']),
        userIsLoggedOn() {
    ...
    methods: {
        ...
        setMathPlugin() {
            window.renderMathInElement(document.getElementById('test_solving_component'), {
                delimiters: [
        ...
        isCourseInBasket(courseId) {
            if (this.isBoughtCourse(courseId))
                return true;
            else if (this.coursesOnBasket.find(item => parseInt(item.id) === parseInt(courseId)))
                return true;
            return false;
        },
        isBoughtCourse(courseId) {
            return !!this.boughtCourses.find(course => parseInt(course.id) === parseInt(courseId));
        },
        isCourseInFavouritesList(courseId) {
            return !!this.favouriteCourses.find(course => parseInt(course.id) === parseInt(courseId));
        },
        isCourseInBasketSolid(courseId) {
            return !!this.coursesOnBasket.find(item => parseInt(item.id) === parseInt(courseId))
        },
